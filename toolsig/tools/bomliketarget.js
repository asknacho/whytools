...
},
{
  type:'input',
  name:'target',
  message:'Username Target (Langsung username yang mau lu bom[at]):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
...
},
{
  type:'input',
  name:'sleep',
  message:'Dellay berapa ( In MiliSeconds):',
  validate: function(value){
    value = value.match(/[0-9]/);
    if (value) return true;
...
const Excute = async function(User, TargetUsername, ittyw, sleep){
  try {
    ...
    console.log(chalk`{bold.green [✓] Login Succsess} {yellow [?] Sedang mendapatkan semua ID media . . .}`);  
    ...
    console.log(chalk`{bold.green [✓] Like Succeeded [SUCCES]\n}`)
  } catch (err) {
    console.log(err);
  }
}
console.log(chalk`{bold.green
  ...
  Ξ CODE   : WHYTOOLS 55232 (officialwhytools)
