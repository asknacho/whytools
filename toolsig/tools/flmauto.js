...
},
{
	type:'input',
	name:'text',
	message:'Masukan Text Comment (Use [|] if more than 1):',
	validate: function(value){
		if(!value) return 'Can\'t Empty';
		return true;
	}
...
const Excute = async function(User, TargetUsername, Text, Sleep, ittyw){
	try {
		...
		console.log(chalk`{green [✓] Login Succsess. }{yellow [?] Sedang Mendapatkan Link & Media ID Target . . .}`)
		...
		console.log(chalk`{yellow [#][>] START FLMAUTO WITH RATIO ${ittyw} TARGET/${Sleep} DETIK [<][#]\n}`)
...
console.log(chalk`{bold.green
  Ξ TITLE  : WHY TOOLS FLM [FOLLOW-LIKE-COMMENT TARGET LAST UP-POST]
