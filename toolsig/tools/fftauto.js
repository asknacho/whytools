...
},
{
  type:'input',
  name:'target',
  message:'Masukan Username Target (LANGSUNG USERNAME NYA[at]):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
},
{
  type:'input',
  name:'text',
  message:'Masukan Text Comment (Pake [|] if more than 1):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
},
{
  type:'input',
  name:'ittyw',
  message:'Input Total Target Berapa (ITTYW):',
  validate: function(value){
...
const Excute = async function(User, TargetUsername, Text, Sleep, ittyw){
  try {
    ...
    console.log(chalk`{green [✓] Login Succsess. }{yellow [?] Sedang mendapatkan ID & Followers Target . . .}`)
    ...
    console.log(chalk`{cyan [?] Sedang Mem Follow, Comment, and Like Followers Target . . . \n}`)
...
console.log(chalk`{bold.green
  Ξ TITLE  : WHY TOOLS [FOLLOW-LIKE-COMMENT TARGET FOLLOWER]
  Ξ CODE   : WHY TOOLS 32529 (officialwhytools)
