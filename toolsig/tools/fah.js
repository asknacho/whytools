...
},
{
  type:'input',
  name:'hastag',
  message:'Masukan Hashtag (Without #):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
...
const doMain = async (account, hastag, sleep, text, ittyw) => {
  ...
  console.log(chalk`{bold.green [✓] Login Succsess}\n{yellow [?] Sedang mem Follow, Like and Comment Semua Akun Yang Di Hashtag: #${hastag}}`);
  const feed = new Client.Feed.TaggedMedia(account.session, hastag);
  try {
    var cursor;
    var count = 0;
    console.log(chalk`{yellow \n[#][>] START WITH RATIO ${ittyw} TARGET/${sleep} Detik [<][#]\n}`)
    do {
...
console.log(chalk`{bold.green
  Ξ TITLE  : WHY TOOLS [FOLLOW-LIKE-COMMENT TARGET HASTAG]
  Ξ CODE   : WHYTOOLS 15262 (officialwhytools)
